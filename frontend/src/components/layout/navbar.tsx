...
export function Navbar() {
  ...
  const [sellerRedirectUrl, setSellerRedirectUrl] = useState<
    string | undefined
  >(undefined);
  ...
  const [sellerDropdownTimeout, setSellerDropdownTimeout] =
      ...
      <nav className="sticky top-0 z-50 bg-white/60 backdrop-blur-sm shadow-sm border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-2 sm:px-4">
          <div className="flex justify-between items-center h-24">
            ...
            <div className="hidden min-[966px]:flex items-center gap-6">
              {/* Navigation Links */}
              ...
              <div className="flex items-center gap-6">
                {isAuthenticated && !isAuthLoading ? (
                  ...
                  <div className="flex items-center gap-4">
                    <Link
                      href="/buyer/cart"
                      className="w-10 h-10 bg-yogreet-light-gray rounded-full flex items-center justify-center hover:bg-yogreet-light-gray/80 transition-colors cursor-pointer relative"
                    >
                      <FiShoppingCart className="w-5 h-5 text-yogreet-charcoal" />
                      {/* Cart badge */}
                      <span className="absolute -top-1 -right-1 w-5 h-5 bg-yogreet-red text-white text-xs rounded-full flex items-center justify-center">
                        3
                      </span>
                    </Link>
                    <Link
                    ...
                    >
                      ...
                      {/* Samples badge */}
                      <span className="absolute -top-1 -right-1 w-5 h-5 bg-yogreet-sage text-white text-xs rounded-full flex items-center justify-center">
                        5
                      </span>
                    </Link>
                  </div>
                        ...
                        <div className="py-2">
                          ...
                          {user && (
                            <div className="px-4 py-2 border-b border-gray-200">
                              <p className="text-sm font-medium text-yogreet-charcoal truncate">
                                {user.name}
                              </p>
            ...
            {/* Mobile Menu Button */}
            <button
              className="p-2 cursor-pointer block min-[966px]:hidden"
          ...
          {/* Mobile Navigation */}
          {isOpen && (
            <div className="pb-4 space-y-2 min-[966px]:hidden">
              {menuItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  onClick={() => setIsOpen(false)}
                  className="block pl-8 pr-4 py-2 text-yogreet-charcoal font-inter text-base hover:bg-yogreet-light-gray cursor-pointer"
                >
                  {item.label}
                </Link>
              ))}
              <div className="px-4 pt-2 space-y-2">
                {isAuthenticated && !isAuthLoading ? (
                  ...
                  <div className="flex gap-4 mb-4">
                    <Link
                      href="/buyer/cart"
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-yogreet-light-gray rounded-lg hover:bg-yogreet-light-gray/80 transition-colors cursor-pointer relative"
                    >
                      <FiShoppingCart className="w-4 h-4 text-yogreet-charcoal" />
                      <span className="text-yogreet-charcoal font-manrope font-medium">
                        Cart
                      </span>
                      <span className="absolute -top-1 -right-1 w-5 h-5 bg-yogreet-red text-white text-xs rounded-full flex items-center justify-center">
                        3
                  ...
                  >
                    <button
                      onClick={handleBecomeSellerClick}
                      className="w-full inline-flex items-center justify-start gap-2 px-4 py-2 text-yogreet-charcoal text-base font-manrope font-medium hover:text-yogreet-red transition-colors cursor-pointer"
                    >
                      Become Seller
                      <FiChevronDown className="w-4 h-4" />
                    </button>
                ...
                {isAuthenticated && !isAuthLoading ? (
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      onClick={handleLoginClick}
                      className="w-full px-4 py-2 text-left text-base text-yogreet-charcoal font-manrope font-medium hover:text-yogreet-red transition-colors cursor-pointer"
                    >
                      Login
                    </button>
                    <button
                      onClick={handleSignupClick}
                      className="w-full px-4 py-2 border border-yogreet-purple text-base text-yogreet-purple font-manrope font-medium hover:bg-yogreet-purple hover:text-white transition-all cursor-pointer rounded-sm text-left"
                    >
                      Sign Up
                    </button>
                  </div>
