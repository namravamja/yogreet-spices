...
interface Step4Props {
  data: SellerShippingData & Record<string, any>;
  updateData: (updates: Partial<SellerShippingData>) => void;
  isLoading?: boolean;
}
...
export default function Step4ShippingLogistics({ data, updateData }: Step4Props) {
  const handleServiceAreaToggle = (area: string) => {
    const currentAreas = Array.isArray(data.serviceAreas) ? data.serviceAreas : [];
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        {Array.isArray(data.serviceAreas) && data.serviceAreas.length > 0 && (
