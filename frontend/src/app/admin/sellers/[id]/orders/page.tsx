...
import { useGetSellerByIdQuery, useGetSellerOrdersByIdQuery } from "@/services/api/adminApi"
import { FiArrowLeft, FiShoppingCart, FiPackage, FiUser, FiMapPin, FiCalendar, FiCreditCard } from "react-icons/fi"
import Image from "next/image"
import Link from "next/link"

export default function SellerOrdersPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const { data: seller, isLoading: sellerLoading } = useGetSellerByIdQuery(id)
  const { data: orders, isLoading: ordersLoading } = useGetSellerOrdersByIdQuery(id)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-poppins font-semibold text-yogreet-charcoal">
                {seller?.companyName || seller?.fullName || "Seller"}&apos;s Orders
              </h1>
        ...
        {!orders || orders.length === 0 ? (
          <div className="bg-white border border-yogreet-light-gray p-12 text-center shadow-sm rounded-lg">
            ...
            <p className="text-yogreet-warm-gray font-inter">No orders found for this seller</p>
          </div>
        ) : (
          <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-3">
                      ...
                      {order.buyer ? (
                        ...
                        <p className="text-sm text-yogreet-warm-gray">No buyer info</p>
                      )}
                    </div>
